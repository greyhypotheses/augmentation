...
class Preliminaries:
    ...
    def __init__(self, var):
        ...
        # Reads-in the dictionary of logging keys, which is provided by logs.py (import logs)
        logging.config.dictConfig(logs.logs)
        self.logger = logging.getLogger('debug')
        self.logger.disabled = True
        self.logger.name = __name__

        self.var = var
        self.minimum_class_instances = self.var.modelling.minimum_class_instances

    def instances(self, inventory: pd.DataFrame, labels: list) -> pd.Series:
        """
        This method calculates the number of instances/records per class w.r.t. a data set
        whose labels have been one-hot-coded.

        :param inventory: A data frame of data
        ...
        summary: pd.Series = inventory[labels].sum()
    ...
    def exc(self) -> (pd.DataFrame, typing.List, typing.List):
        """
        ...
        inventory, fields, labels = sources.Sources(var=self.var).exc()

        # In terms of the data in question, how many instances are there per class?
        instances_per_class: pd.Series = self.instances(inventory=inventory, labels=labels)
